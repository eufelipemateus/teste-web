...
const validationSchema = yup.object().shape({
  qtd: yup.number().integer().positive().min(1).required(),
  valor: yup.number().round('ceil').positive().required(),
  description: yup.string().required().min(15).max(200)
});
...
const FormDialog = memo((props: IProps) => {
  ...
  const formik = useFormikObservable<IOrder>({
    initialValues: { qtd: 1, valor: 0, description: '' },
    validationSchema,
    onSubmit(model) {
      model.qtd = Number(model.qtd);
      model.valor = Number(model.valor);
      return orderService.save(model).pipe(
        tap(order => {
          Toast.show(`${model.id ? 'O pedido Foi salvo!' : 'O pedido foi criado'}`);
            ...
            <Fragment>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <TextField label='Quantidade' value='1' name='qtd' type='number' formik={formik} />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField
                    label='Valor'
              ...
              <TextField label='Description' name='description' multiline rows={4} formik={formik} />
